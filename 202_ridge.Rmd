...
data.frame(
  glmnet = glmnet(X, Y, alpha=0, lambda=0.5, thresh = 1e-16, intercept = F, standardize=F)$beta[,1],
  ridge = ridge(X, Y, lambda=0.5)
)

data.frame(
  glmnet = glmnet(X_standard, Y, alpha=0, lambda=0.5, thresh = 1e-16, intercept = F, standardize=F)$beta[,1],
  ridge = ridge(X_standard, Y, lambda=0.5)
)
